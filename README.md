...
- [Saifuro](https://saifuro.com) - Non-custodial authorization layer for agent payments. Policy is evaluated per request and returns an ES256-signed verdict binding amount, recipient, mandate and policy version; the protocol field records x402, ACP, AP2 or card-rail on the decision. Funds never pass through it. API reference, OpenAPI and signing keys are public, no signup. ([Keys](https://verdicts.saifuro.com/.well-known/jwks.json))
